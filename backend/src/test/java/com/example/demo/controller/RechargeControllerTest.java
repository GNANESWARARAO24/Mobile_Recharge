...
public class RechargeControllerTest {
	...
	void testRechargeSuccess() throws Exception {
		...
		doNothing().when(emailService).sendConfirmationEmail(anyString(), anyString(), anyString(), anyDouble(),
				anyString());

		mockMvc.perform(post("/api/user/recharge").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(rechargeRequest))).andExpect(status().isOk())
				.andExpect(jsonPath("$.transactionId").exists());

		verify(emailService, times(1)).sendConfirmationEmail(anyString(), anyString(), anyString(), anyDouble(),
				anyString());
	...
	void testRechargeInvalidCardDetails() throws Exception {
		...
		mockMvc.perform(post("/api/user/recharge").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(rechargeRequest))).andExpect(status().isBadRequest())
				.andExpect(jsonPath("$").value("Invalid card details"));

		verify(emailService, never()).sendConfirmationEmail(anyString(), anyString(), anyString(), anyDouble(),
				anyString());
	}
	...
	void testRechargeSubscriberNotFound() throws Exception {
		...
		mockMvc.perform(post("/api/user/recharge").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(rechargeRequest))).andExpect(status().isNotFound())
	...
	void testRechargeEmailFailure() throws Exception {
		...
		when(rechargeRepository.save(any())).thenAnswer(i -> i.getArgument(0));
		when(subscriberRepository.save(any())).thenAnswer(i -> i.getArgument(0));
		doThrow(new RuntimeException("Email failure")).when(emailService).sendConfirmationEmail(anyString(),
