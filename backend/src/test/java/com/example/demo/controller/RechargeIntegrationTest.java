...
public class RechargeIntegrationTest {
	...
	void testRechargeFullFlow() throws Exception {
		...
		mockMvc.perform(post("/api/user/recharge").contentType(MediaType.APPLICATION_JSON).content(rechargeRequest))
				.andExpect(status().isOk()).andExpect(jsonPath("$.transactionId").exists());
	}
	...
	void testRechargeWithInvalidCardDetails() throws Exception {
		...
		mockMvc.perform(post("/api/user/recharge").contentType(MediaType.APPLICATION_JSON).content(rechargeRequest))
				.andExpect(status().isBadRequest()).andExpect(jsonPath("$").value("Invalid card details"));
	}
}